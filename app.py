...
with tab5:
    ...
    if team_a_name == team_b_name:
        ...
        col_labels = {"game_day": "Date", "opponent": "Opponent", "location": "Location",
            ...
            with res_col:
                ...
                try:
                    ...
                    if results_df.empty:
                        ...
                        def _color_rows(row):
                            if str(row["Result"]).startswith("W"):
                                return ["background-color: rgba(34, 197, 94, 0.15)"] * len(row)
                            elif str(row["Result"]).startswith("L"):
                                return ["background-color: rgba(239, 68, 68, 0.15)"] * len(row)
                            return [""] * len(row)
                        st.dataframe(
                            results_df.style.apply(_color_rows, axis=1),
                            use_container_width=True,
                            hide_index=True,
                        )
                except Exception:
